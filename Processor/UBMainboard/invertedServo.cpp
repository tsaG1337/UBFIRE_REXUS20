...
void Servo_Handler(timer16_Sequence_t timer, Tc *tc, uint8_t channel)
{
    ...
    if (Channel[timer] < 0) {
        ...
        if (SERVO_INDEX(timer,Channel[timer]) < ServoCount && SERVO(timer,Channel[timer]).Pin.isActive == true) {
            digitalWrite(SERVO(timer,Channel[timer]).Pin.nbr, HIGH); // pulse this channel low if activated CHANGED BY PAT
        }
    }
    ...
    if( SERVO_INDEX(timer,Channel[timer]) < ServoCount && Channel[timer] < SERVOS_PER_TIMER) {
        ...
        if(SERVO(timer,Channel[timer]).Pin.isActive == true) {    // check if activated
            digitalWrite( SERVO(timer,Channel[timer]).Pin.nbr,LOW); // its an active channel so pulse it high CHANGED BY PAT
        }
    }
    else {
